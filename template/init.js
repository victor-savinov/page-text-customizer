    ...
    Satus.locale(function() {
        ...
        document.querySelector('.satus').addEventListener('satus-navigate', function(event) {
            if (event.detail.name === 'main') {
                document.querySelector('.satus-header__title').innerText = 'Replace Text';
                ...
                for (var i = 0, l = event.detail.path.length; i < l; i++) {
                    ...
                    if (Satus.memory.has('locale/' + part)) {
        ...
        /*if (Satus.storage.get('')) {
            ...
            for (var key in replace_data) {
                replace_value += '"' + key + '" => "' + replace_data[key] + '"\n\r';
            }

            Menu.main.section.textField.value = replace_value;
        }*/
